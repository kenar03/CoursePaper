...
* форматирование напряжение в строку <<output_format, заданного формата>> (класс *Formatter*);
...
Блок управления светодиодами состоит из двух независимых друг от друга классов *LedCalculator* и *LedController*.
...
Класс *LedController* осуществляет <<leds, управление непосредственно светодиодами>>. У него есть единственный метод *Indicate*, который принимает в качестве аргумента количество светодиодов, которые необходимо включить. Этот класс наследуется от интерфейса *ILedCalculator*.
